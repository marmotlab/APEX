...
class G1Robot(LeggedRobot):
	...
	def _compute_torques(self, actions):
		...
		if control_type == 'apex_torque':
			...
			decap_factor = gamma_decap**(self.torque_ref_decay_factor/k_decap)
			torques = self.p_gains*self.Kp_factors*(self.joint_pos_target - self.dof_pos + self.motor_offsets) - self.Kd_factors*self.d_gains*self.dof_vel + decap_factor*(self.p_gains*(dof_imit_arr - self.dof_pos)- self.d_gains*self.dof_vel)
	...
	def _reset_dofs(self, env_ids):
		...
		if not reference_state_init:
			...
			#Convert to tensor
			self.reference_state_angles = torch.from_numpy(self.df_imit.iloc[self.imitation_index.detach().cpu().numpy().astype(int),6:29].to_numpy()).float().to(self.device)
	...
	def _reset_root_states(self, env_ids):
		...
		if reference_state_init:
			self.reference_velocity = torch.from_numpy(self.df_imit.iloc[self.imitation_index.detach().cpu().numpy().astype(int),0:6].to_numpy()).float().to(self.device)
			self.root_states[env_ids, 7:13] = self.reference_velocity[env_ids]  # [7:10]: lin vel, [10:13]: ang vel
			self.root_states[env_ids, 7:13] = torch_rand_float(-0.5, 0.5, (len(env_ids), 6), device=self.device) # [7:10]: lin vel, [10:13]: ang vel
			self.reference_state_height = torch.from_numpy(self.df_imit.iloc[self.imitation_index.detach().cpu().numpy().astype(int),29].to_numpy()).float().to(self.device)
			self.root_states[env_ids, 2] = self.reference_state_height[env_ids]  # [2]: height
			self.reference_quaternions = torch.from_numpy(self.df_imit.iloc[self.imitation_index.detach().cpu().numpy().astype(int),38:42].to_numpy()).float().to(self.device)
	...
	def _reward_imitate_end_effector_pos(self):
		index_array = self.imitation_index.detach().cpu().numpy().astype(int)
		end_effector_ref = self.df_imit.iloc[index_array,30:36].to_numpy()
		...
		# return torch.exp(-80*end_effect/or_error)
	...
	def _reward_imitation_angles(self):
		...
		return torch.exp(-40*dof_imit_error)  
	
	def _reward_imitation_height_penalty(self):
		index_array = self.imitation_index.detach().cpu().numpy().astype(int)
		# print("Imitation Index", index_array[0])
		# print(index_array)
		# Retrieve the corresponding rows from df_imit using array indexing
		height = self.df_imit.iloc[index_array,29].to_numpy()
